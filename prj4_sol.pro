...
% are sub-lists.
sublist_lengths([], []).
sublist_lengths([Head|Tail], [LengthHead|LengthTail]) :-
    length(Head, LengthHead),
    sublist_lengths(Tail, LengthTail).
...
same_length_sublists([Head|Tail]) :-
    sublist_length(Head, Length),
    check_sublist_lengths(Tail, Length).

sublist_length([], 0).
sublist_length([_|T], Length) :-
    sublist_length(T, RemainingLength),
    Length is RemainingLength + 1.

check_sublist_lengths([], _).
check_sublist_lengths([Head|Tail], Length) :-
    sublist_length(Head, Length),
    check_sublist_lengths(Tail, Length).
...
assoc_replace([Head|Tail], Assoc, [Value|Values]) :-
    ...
    assoc_replace(Tail, Assoc, Values).
...
named_to_op_expr(add(X, Y), X_ + Y_) :-
    named_to_op_expr(X, X_),
    named_to_op_expr(Y, Y_).

named_to_op_expr(mul(X, Y), X_ * Y_) :-
    named_to_op_expr(X, X_),
    named_to_op_expr(Y, Y_).
...
named_expr_to_prefix_tokens(NamedExpr, [NamedExpr]) :-
    integer(NamedExpr).

named_expr_to_prefix_tokens(add(A, B), [add|Tokens]) :-
    named_expr_to_prefix_tokens(A, ATokens),
    named_expr_to_prefix_tokens(B, BTokens),
    append(ATokens, BTokens, Tokens).

named_expr_to_prefix_tokens(mul(A, B), [mul|Tokens]) :-
    named_expr_to_prefix_tokens(A, ATokens),
    named_expr_to_prefix_tokens(B, BTokens),
    append(ATokens, BTokens, Tokens).
